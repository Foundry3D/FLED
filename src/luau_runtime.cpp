...
namespace {
    ...
    static std::string normalizePath(lua_State *L, const std::string &path) {
        ...
        std::filesystem::path normalized_path = (source_dir / path).lexically_normal();
    ...
    static int lua_require(lua_State *L) {
        ...
        if (luau_load(ML, resolved_path.c_str(), bytecode.data(), bytecode.size(), 0) == 0) {
            // NOTICE: Call debugger when file is loaded
            auto *debugger = reinterpret_cast<luau::debugger::Debugger *>(lua_getthreaddata(GL));
...
namespace luau {
    ...
    Runtime::Runtime() {
        Luau::assertHandler() = [](const char *expr, const char *file, int line, const char *function) {
            if (runtime_)
                runtime_->onError(std::format("{}({}): ASSERTION FAILED: {}\n", file, line, expr), nullptr);
            return 1;
        };
        runtime_ = this;
        vm_ = luaL_newstate();
